...
//    private final long startTime; // Changed from endTime to startTime as per Javadoc
//    private final UI ui;
//    private final GameModel model;
//    private final AchievementManager achievementManager; // Changed from aManager to achievementManager
...
//        int shotsHit = getStatsTracker().getShotsHit();
//        int shotsFired = getStatsTracker().getShotsFired();
//        double accuracy = getStatsTracker().getAccuracy();
//
//        // Update Survivor achievement (mastered at 120 seconds)
//        double survivorProgress = Math.min(1.0, elapsedSeconds / 120.0);
//        achievementManager.updateAchievement("Survivor", survivorProgress);
//
//        // Update Enemy Exterminator achievement (mastered at 20 shots hit)
...
//
//        // Update Sharp Shooter achievement
...
//                ui.log(String.format("%s: %.2f%% (Tier: %s)",
//                        achievement.getName(),
//                        achievement.getProgress() * 100,
//                        achievement.getCurrentTier()));
//            }
//        }
//    }
//
...
//        ui.setStat("Time Survived", String.valueOf((System.currentTimeMillis() - startTime) / 1000) + " seconds");
//
//        // Create a NEW list with all objects in the correct order
//        List<SpaceObject> allObjects = new ArrayList<>();
//
//        // Add the ship FIRST for proper layering
//        allObjects.add(model.getShip());
//
//        // Add all other game objects
//        allObjects.addAll(model.getSpaceObjects());
//
//        // Render all space objects including ship
...
public class GameController {
    ...
    /**
     * Sets verbose state to the provided input. Also sets the model's verbose state to the provided input.
     * @param verbose whether to set verbose state to true or false.
     */
    public void setVerbose(boolean verbose) {
        this.isVerbose = verbose;
     ...
     * - Sharp Shooter achievement: if shots fired > 10, then result is accuracy / 0.99, with the maximum
     *   result possible being 1; otherwise if shots fired <= 10, result is 0.
     * (This is so that mastery is achieved at accuracy >= 0.99)
     *
     * The AchievementManager stores all new achievements mastered, and then updates the UI statistics
     ...
     *   unless the game is paused. The movement should also be logged, provided verbose is true.
    ...
    private void showGameOverWindow() {
        ...
        sb.append("Survival Time: ").append(statsTracker.getElapsedSeconds()).append(" seconds\n");
